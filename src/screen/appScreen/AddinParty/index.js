...
const AddinParty = (props) => {
    ...
    const methodSyncContact = (perData) => {
        ...
        dispatch(addPartyMembers(dic)).then((res) => {
            if (res?.status) {
    ...
    const sendSelectedUsers = () => {
        ...
        dispatch(addUsersInPartyapi(dic)).then((res) => {
            if (res?.status) {
                dispatch(loadingShow(false));
                props.navigation.goBack();
            }
            ...
            <View key={index} style={styles.card}>
                <View style={styles.flatlistmainview} >
                    ...
                    {item?.profile_picture ?
                        ...
                        <ImageLoadView style={styles.nameview} source={{
                            uri: IMAGE_URL + item?.profile_picture,
                        }} /> :
                        <View style={styles.nameview}>
                            <Text style={[styles.firstLetterText, { color: 'white', fontFamily: fonts.Montserrat_Bold, fontSize: fonts.SIZE_20 }]}>{firstLetter}</Text>
                        </View>}
                    <View style={styles.imageview} >
                        <Text style={[styles.name, { width: '70%', fontFamily: fonts.Montserrat_Bold }]}>{item.name ? item?.name : item?.phone_number}</Text>
                        <Text style={[styles.name, { fontFamily: fonts.Montserrat_Regular, color: '#777', flexGrow: 1, marginTop: -15 }]}>{item?.address}</Text>
                        <Text style={[styles.name, { fontFamily: fonts.Montserrat_Regular, color: '#777', flexGrow: 1, marginTop: -15 }]}>{item?.gender}</Text>
                    </View>
    ...
    return (
        <View style={styles.container} >
            ...
            <FlatList style={{ flex: 1, marginTop: 20 }}
                ...
                ListEmptyComponent={
                    !isLoading ? (
                        <NoDataFound NoData={"No members found..!"} />
                    ) : (
                        <View />
                    )
                }
