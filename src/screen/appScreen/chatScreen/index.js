...
import {IMAGE_URL, MULTI_PART_HEADER} from '../../../appRedux/apis/commonValue';
...
const ChatScreen = props => {
  ...
  useEffect(() => {
    socketConnectionCheck();
     AppHeader({
      ...
      Title: translateText('Chat_Screen'),
      notificationClick: () => {
  ...
  useEffect(() => {
    ...
    let blockUserListener = DeviceEventEmitter.addListener(
      'user_blocked',
      res => {},
    );
  ...
  const methodGetAllChat = (listType = 'before') => {
    ...
    socketEmit(socketEvent.getchats, dic, res => {
      setIsLoading(false);
  ...
  const methodSendMessage = text => {
    ...
    setMessage('');
    socketEmit(socketEvent.send_message, dic, res => {
      if (res && res?.data) {
        messageData = [res?.data, ...messageData];
        setChatHistoryData(messageData);
        scrollToTop();
  ...
  const sendMedia = async (imgData, type) => {
    ...
    try {
      ...
      if (response && response.status === true) {
        ...
        showToastMessage(response?.message || 'Upload failed');
      }
    } catch (error) {
      dispatch(loadingShow(false));
    ...
    <KeyboardAvoidingView
      ...
      <View style={styles.container}>
                ...
                <TouchableOpacity
                  onPress={
                    () => {
                      ...
                      //   // { text: "Video", onPress: () => chooseImage("video") },
        ...
        <UserInfoModal
          ...
          proname={
            providerAllData?.data?.name
              ? providerAllData?.data?.name
              : providerAllData?.provider_detail?.name
          }
          user_icon={
            providerAllData?.data?.profile_picture
              ? IMAGE_URL + providerAllData?.data?.profile_picture
              : IMAGE_URL + providerAllData?.provider_detail?.profile_picture
          }
          onCancel={() => {
